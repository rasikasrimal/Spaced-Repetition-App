...
export const TopicHistoryEditor: React.FC<TopicHistoryEditorProps> = ({
  ...
  const [isMounted, setIsMounted] = React.useState(isModal ? false : true);
  ...
  React.useEffect(() => {
    ...
    const overflow = Math.max(0, sorted.length - MAX_HISTORY_ENTRIES);
    setOverflowCount(overflow);
    setDrafts(overflow > 0 ? sorted.slice(-MAX_HISTORY_ENTRIES) : sorted);
  }, [isModal, open, topic, timezone]);
  ...
  const addDraft = React.useCallback(() => {
    ...
    setDrafts((previous) => {
      ...
      const next = sortDrafts([...previous, draft], timezone);
      overflow = Math.max(0, next.length - MAX_HISTORY_ENTRIES);
      return overflow > 0 ? next.slice(overflow) : next;
    });
    if (overflow > 0) {
      setOverflowCount((count) => count + overflow);
      toast.info(
        overflow === 1
          ? `Oldest entry hidden to keep the latest ${MAX_HISTORY_ENTRIES}.`
          : `${overflow} oldest entries hidden to keep the latest ${MAX_HISTORY_ENTRIES}.`
      );
    }
  }, [timezone]);
  ...
  const applyBulkInput = React.useCallback(() => {
    ...
    let overflow = 0;
    setDrafts((previous) => {
      const next = sortDrafts([...previous, ...additions], timezone);
      overflow = Math.max(0, next.length - MAX_HISTORY_ENTRIES);
      return overflow > 0 ? next.slice(overflow) : next;
    });
    if (overflow > 0) {
  ...
  if (!isModal) {
    return (
      <>
        <div className="mx-auto w-full max-w-5xl px-4 py-10">{pagePanel}</div>
        <ConfirmationDialog
