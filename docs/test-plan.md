...
| Timeline fullscreen availability | Enter fullscreen from Combined view, exit, switch to Per subject and repeat | Fullscreen button stays visible, opens the current chart without losing zoom/filters, and escape restores layout without layout jumps. |
| Revision tables hover | Expand a subject under the timeline, hover numbered badges | Tooltip shows review date, badge gains accent border, and retention percentages remain colour-coded per thresholds. |
